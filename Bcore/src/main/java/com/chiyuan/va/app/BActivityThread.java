...
import com.chiyuan.va.core.CrashHandler;
...
public class BActivityThread extends IBActivityThread.Stub {
    ...
    private final Handler mH = ChiyuanVACore.get().getHandler();
    ...
    public static int getCallingBUid() {
        return getAppConfig() == null ? ChiyuanVACore.getHostUid() : getAppConfig().callingBUid;
    }
    ...
    public Service createService(ServiceInfo serviceInfo, IBinder token) {
        ...
        try {
            Context context = ChiyuanVACore.getContext().createPackageContext(
    ...
    public synchronized void handleBindApplication(String packageName, String processName) {
        ...
        PackageInfo packageInfo = ChiyuanVACore.getBPackageManager().getPackageInfo(packageName, PackageManager.GET_PROVIDERS, BActivityThread.getUserId());
        ApplicationInfo applicationInfo = packageInfo.applicationInfo;
...
VirtualRuntime.setupRuntime(processName, applicationInfo);
        ...
        try {
            ...
            ContextCompat.fix((Context) BRActivityThread.get(ChiyuanVACore.mainThread()).getSystemContext());
    ...
    private void setApplication(Application application) {
        try {
            mInitialApplication = application;
            BRActivityThread.get(ChiyuanVACore.mainThread())._set_mInitialApplication(application);
            ...
            for (ProviderInfo providerInfo : providers) {
                try {
                    if (processName.equals(providerInfo.processName) ||
                        ...
                        installProvider(ChiyuanVACore.mainThread(), context, providerInfo, null);
                    }
    ...
    public IBinder getActivityThread() {
        return BRActivityThread.get(ChiyuanVACore.mainThread()).getApplicationThread();
    }
    ...
    public IBinder acquireContentProviderClient(ProviderInfo providerInfo) throws RemoteException {
        ...
        for (String auth : split) {
            ContentProviderClient contentProviderClient = ChiyuanVACore.getContext()
    ...
    public void finishActivity(final IBinder token) {
        mH.post(() -> {
            Map<IBinder, Object> activities = BRActivityThread.get(ChiyuanVACore.mainThread()).mActivities();
    ...
    public void scheduleReceiver(ReceiverData data) throws RemoteException {
        ...
        mH.post(() -> {
            ...
            try {
                ...
                ChiyuanVACore.getBActivityManager().finishBroadcast(data.data);
    ...
    public static Activity getActivityByToken(IBinder token) {
        Map<IBinder, Object> iBinderObjectMap =
                BRActivityThread.get(ChiyuanVACore.mainThread()).mActivities();
        ...
        try {
            ...
            try {
                ...
                if (validContext == null) {
                    validContext = ChiyuanVACore.getContext();
                }
            } catch (Exception e) {
